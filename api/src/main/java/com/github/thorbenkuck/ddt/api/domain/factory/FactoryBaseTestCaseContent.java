...
public class FactoryBaseTestCaseContent<T, S> extends AbstractTestCaseContent<FactoryBasedTestCaseInput<T, S>, FactoryBasedTestCaseOutput<S>> {

    private final Class<? extends TestContentFactory<T, S>> factoryType;
    private final TestCaseEntry<T, S> factoryResult;

    public FactoryBaseTestCaseContent(
            Class<? extends TestContentFactory<T, S>> factoryType,
            TestCaseEntry<T, S> factoryResult,
            TestScenario annotation
    ) {
        super(new FactoryBasedTestCaseInput<>(factoryResult), annotation);
